...
class TestHealthEndpoint:
    ...
    def test_uptime_is_number(self, client, mock_scored_df):
    ...
    def test_version_field_present(self, client, mock_scored_df):
...
class TestApiScores:
    ...
    def test_filter_by_decision(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/api/scores?decision=APPROVE")
            body = resp.get_json()
            assert body["count"] == 3
            assert all(d["decision"] == "APPROVE" for d in body["data"])

    def test_filter_by_state(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/api/scores?state=KARNATAKA")
            body = resp.get_json()
            assert body["count"] == 2

    def test_combined_filters(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/api/scores?commodity=WHEAT&decision=APPROVE&state=MAHARASHTRA")
            body = resp.get_json()
            assert body["count"] == 2

    def test_limit_parameter(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
    ...
    def test_limit_clamped_to_max(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/api/scores?limit=99999")
            body = resp.get_json()
            # limit is clamped to 10000, but we only have 6 rows
            assert body["count"] == 6

    def test_response_schema(self, client, mock_scored_df):
...
class TestApiScoresCommodity:
    ...
    def test_returns_commodity_data(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/api/scores/WHEAT")
            assert resp.status_code == 200
            body = resp.get_json()
            assert body["commodity"] == "WHEAT"
            assert body["count"] == 3

    def test_avg_risk_and_ltv_present(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/api/scores/WHEAT")
            body = resp.get_json()
    ...
    def test_decisions_dict_present(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/api/scores/WHEAT")
            body = resp.get_json()
    ...
    def test_not_found_returns_404(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
    ...
    def test_case_insensitive(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/api/scores/wheat")
            body = resp.get_json()
            assert body["commodity"] == "WHEAT"
...
class TestApiSummary:
    ...
    def test_returns_summary(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
    ...
    def test_risk_percentiles(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/api/summary")
            body = resp.get_json()
    ...
    def test_decision_distribution(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/api/summary")
            body = resp.get_json()
    ...
    def test_avg_ltv_is_percentage(self, client, mock_scored_df):
...
class TestApiDecisions:
    ...
    def test_returns_decision_distribution(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
    ...
    def test_records_have_commodity_and_decision(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/api/decisions")
            body = resp.get_json()
...
class TestApiCommodities:
    ...
    def test_returns_commodity_list(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
    ...
    def test_commodity_stats(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/api/commodities")
            body = resp.get_json()
            record = body["data"][0]
    ...
    def test_sorted_by_count_descending(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/api/commodities")
            body = resp.get_json()
...
class TestHtmlRoutes:
    ...
    def test_dashboard_renders(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/")
            assert resp.status_code == 200
            assert b"<html" in resp.data.lower() or b"<!DOCTYPE" in resp.data

    def test_commodity_detail_renders(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/commodity/WHEAT")
            assert resp.status_code == 200

    def test_commodity_detail_404(self, client, mock_scored_df):
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/commodity/NONEXISTENT")
            assert resp.status_code == 404

    def test_commodity_hyphen_to_space(self, client, mock_scored_df):
        """URL hyphens should be converted to spaces for matching."""
        with patch("src.api.app._scored_df", mock_scored_df):
            resp = client.get("/commodity/GREEN-CHILLI")
